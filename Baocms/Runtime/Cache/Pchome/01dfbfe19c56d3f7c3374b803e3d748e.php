...
<div class="topTwo">
    <div class="left">
        <?php if(empty($CONFIG['site']['logo'])): ?><h1><a href="<?php echo U('pchome/index/index');?>"><img src="__ROOT__/attachs/<?php echo ($CONFIG["site"]["logo"]); ?>" /></a></h1>
            <?php else: ?>
            <h1><a href="<?php echo U('pchome/index/index');?>"><img src="__TMPL__statics/images/images/logo.png" /></a></h1><?php endif; ?> 
    ...
    <div class="navList">
        <ul>
        	<li class="navListAll">全部分类</li>
                ...
                <li class="navLi"><a  class="navA " href="http://yanan.rztaiyang.com/mall">商城</a></li>
...
<div class="pagewd" id="index-gun">
    ...
    <div class="sy_cityBuy_nr">
        <?php  $cache = cache(array('type'=>'File','expire'=> 7200)); $token = md5("Ad, closed=0 AND site_id=46 AND city_id IN ({$city_ids})  and bg_date <= '{$today}' AND end_date >= '{$today}' ,0,1,7200,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" closed=0 AND site_id=46 AND city_id IN ({$city_ids})  and bg_date <= '{$today}' AND end_date >= '{$today}' ")->order("orderby asc")->limit("0,1")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><div class="left sy_cityBuy_ad"><a target="_blank" href="<?php echo ($itme["link_url"]); ?>"><img src="__ROOT__/attachs/<?php echo ($item["photo"]); ?>" width="260" height="499" /></a></div> <?php endforeach; ?>
    ...
    <div class="sy_FloorBt" data="top_3" id="floor3">
        <div class="left sy_FloorBtz"><i class="ico_3"></i><span class="bt cl_2">校园餐厅</span></div>
    	...
    	<div class="sy_waimai_one">
        	<div class="left sy_waimai_cate">
            	<div class="sy_waimai_cate_list">
                    ...
                    <?php  $cache = cache(array('type'=>'File','expire'=> 14400)); $token = md5("Ad, closed=0 AND site_id=47 AND city_id IN ({$city_ids})  and bg_date <= '{$today}' AND end_date >= '{$today}' ,0,1,14400,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" closed=0 AND site_id=47 AND city_id IN ({$city_ids})  and bg_date <= '{$today}' AND end_date >= '{$today}' ")->order("orderby asc")->limit("0,1")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><div class="sy_waimai_ad"><a target="_blank" href="<?php echo ($itme["link_url"]); ?>"><img src="__ROOT__/attachs/<?php echo ($item["photo"]); ?>" width="180" height="180" /></a></div> <?php endforeach; ?>
            </div>
            <div class="left sy_waimai_lunbo">
            	<div class="sy_waimai_flexslider">
                    <ul class="slides">
                        <?php  $cache = cache(array('type'=>'File','expire'=> 43200)); $token = md5("Ad, closed=0 AND site_id=48 AND city_id IN ({$city_ids})  and bg_date <= '{$today}' AND end_date >= '{$today}' ,0,3,43200,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" closed=0 AND site_id=48 AND city_id IN ({$city_ids})  and bg_date <= '{$today}' AND end_date >= '{$today}' ")->order("orderby asc")->limit("0,3")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="list"><a target="_blank" href="<?php echo ($itme["link_url"]); ?>"><img src="__ROOT__/attachs/<?php echo ($item["photo"]); ?>" width="459" height="338" /></a></li> <?php endforeach; ?>
                    </ul>
                </div>
            </div>
        ...
        <div class="sy_waimai_list_box">
        	<ul>
            	<?php  $cache = cache(array('type'=>'File','expire'=> 600)); $token = md5("Ele,audit = 1 AND city_id=$city_id ,1,orderby asc,sold_num desc,2,5,600,"); if(!$items= $cache->get($token)){ $items = D("Ele")->where("audit = 1 AND city_id=$city_id ")->order("orderby asc,sold_num desc")->limit("2,5")->select(); $items = D("Ele")->CallDataForMat($items); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="sy_waimai_list">
                        <div class="img"><a title="<?php echo ($item["shop_name"]); ?>家外卖" target="_blank" href="<?php echo U('ele/shop',array('shop_id'=>$item['shop_id']));?>"><div class="tag_box"><?php if(($item["is_pay"]) == "1"): ?><i class="bg_2">付</i><?php endif; ?>
...
<div class="indexpop" id="fox-food">
    <ul>
        ...
        <li><a href="#floor2"><div class="wz">商城</div></a></li>
        <li><a href="#floor3"><div class="wz">外卖</div></a></li>
        <li><a href="#floor4"><div class="wz">优惠</div></a></li>
        <li><a href="#floor5"><div class="wz">订座</div></a></li>
        <li><a href="#floor6"><div class="wz">信息</div></a></li>
        <li><a href="#floor7"><div class="wz">活动</div></a></li>
        <li><a href="#floor8"><div class="wz">上门</div></a></li>
    </ul>
</div>
<script>
    $(function () {
...
<div class="footerOut">
    <?php if($ctl == index): ?><div class="foot_yqlj">
            友情链接：
            <?php  $cache = cache(array('type'=>'File','expire'=> 21600)); $token = md5("Links,,0,10,21600,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Links")->where("")->order("orderby asc")->limit("0,10")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><a target="_blank" href="#">百朋言欢</a> <?php endforeach; ?>
        </div><?php endif; ?>
    <div class="footer">
        <div class="footNav">
            <div class="left">
                <div class="footNavLi">
                    <ul>
                    	<li class="footerLi foot_contact">
                			...
                			<div class="nr">
                            	<p>客服电话：<b class="fontcl3"></b></p>
                                <p class="graycl">免费长途</p>
                                <p>在线客服：<a target="_blank" href=""/></a></p>
